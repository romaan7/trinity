  ...
  function map() {
    ...
    function map(selection) {
      selection.each(function(data) {
        ...
        var minX = d3.min(_data.raw, function(line$$1) {
          return d3.min(line$$1.nodes, function(node) {
            return node.coords[0];
          });
        });

        var maxX = d3.max(_data.raw, function(line$$1) {
          return d3.max(line$$1.nodes, function(node) {
            return node.coords[0];
          });
        });

        var minY = d3.min(_data.raw, function(line$$1) {
          return d3.min(line$$1.nodes, function(node) {
            return node.coords[1];
          });
        });

        var maxY = d3.max(_data.raw, function(line$$1) {
          return d3.max(line$$1.nodes, function(node) {
            return node.coords[1];
          });
        });
      ...
      data.lines.forEach(function(line$$1) {
        for (var node = 0; node < line$$1.nodes.length; node++) {
          ...
          if (station$$1.labelPos === undefined) {
            ...
            station$$1.labelShiftX = d.hasOwnProperty('labelShiftCoords')
              ? d.labelShiftCoords[0]
              : d.hasOwnProperty('shiftCoords')
                ? d.shiftCoords[0]
                : line$$1.shiftCoords[0];
            station$$1.labelShiftY = d.hasOwnProperty('labelShiftCoords')
          ...
          station$$1.label = data.stations[d.name].label;
